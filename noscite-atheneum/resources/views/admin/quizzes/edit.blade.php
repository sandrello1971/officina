...
<div style="max-width:600px;">
    <div style="margin-bottom:20px;">
        ...
        <h2 style="font-size:1.25rem; font-weight:700; color:#1A1F1F; margin-top:4px;">Modifica: {{ $quiz->title }}</h2>
    </div>
        ...
        <form method="POST" action="{{ route('admin.quizzes.update', $quiz->id) }}">
            ...
            <div style="display:flex; flex-direction:column; gap:16px;">
                <div>
                    ...
                    @error('title')<p style="color:#E28A53; font-size:0.75rem; margin-top:4px;">{{ $message }}</p>@enderror
                </div>
                <div>
                    <label style="font-size:0.8rem; font-weight:600; color:#4A5252; display:block; margin-bottom:6px;">Descrizione</label>
                    <textarea name="description" rows="3"
                              style="width:100%; padding:10px 14px; border:1px solid #C8D0D0; border-radius:8px; font-size:0.875rem; outline:none;">{{ old('description', $quiz->description) }}</textarea>
                </div>
                <div style="display:grid; grid-template-columns:1fr 1fr; gap:16px;">
                    <div>
                        ...
                        <select name="course_id" style="width:100%; padding:10px 14px; border:1px solid #C8D0D0; border-radius:8px; font-size:0.875rem; outline:none;">
                            <option value="">— Nessuno (quiz di modulo) —</option>
                ...
                <div style="display:grid; grid-template-columns:1fr 1fr; gap:16px;">
                    <div>
                        ...
                        <input type="number" name="time_limit_minutes" value="{{ old('time_limit_minutes', $quiz->time_limit_minutes ?? 0) }}" min="0"
                               style="width:100%; padding:10px 14px; border:1px solid #C8D0D0; border-radius:8px; font-size:0.875rem; outline:none;">
                    </div>
                    <div>
                        ...
                        <input type="number" name="max_attempts" value="{{ old('max_attempts', $quiz->max_attempts ?? 0) }}" min="0"
                               style="width:100%; padding:10px 14px; border:1px solid #C8D0D0; border-radius:8px; font-size:0.875rem; outline:none;">
                        <p style="font-size:0.7rem; color:#8A9696; margin-top:4px; line-height:1.4;">
                            0 = tentativi illimitati. Per gli esami (quiz a livello corso) è il numero massimo
                            di tentativi per studente; i tentativi interrotti/abbandonati vengono conteggiati.
                        </p>
                    </div>
                </div>
                ...
                <div style="display:flex; gap:12px; justify-content:flex-end;">
                    <a href="{{ route('admin.quizzes.index') }}" style="padding:10px 20px; border:1px solid #C8D0D0; color:#4A5252; border-radius:8px; font-size:0.875rem; text-decoration:none;">Annulla</a>
