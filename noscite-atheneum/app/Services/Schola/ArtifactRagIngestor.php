...
{
    private const TARGET_CHARS = 420;
    private const MAX_CHARS = 480;
    private const OVERLAP = 60;
    ...
    private function buildChunks(TeachingArtifact $artifact): array
    {
        ...
        if ($artifact->type === 'transcript') {
            ...
            if (is_array($segments) && !empty($segments)) {
                return $this->chunkSegments($segments, $this->sourceRef($doc));
            }
        }
     ...
     * @return list<string>
     */
    private function chunkChars(string $text): array
    {
        $text = trim($text);
        if ($text === '') {
            return [];
     ...
     * @return list<array{content: string, metadata: array}>
     */
    private function chunkSegments(array $segments, array $source): array
    {
        $chunks = [];
        $buf = '';
        $bufStart = null;
