...
<div style="max-width:680px;">
    ...
    <form method="POST" action="{{ route('docente.classi.messaggi.store', $class) }}" data-async style="background:white; border:1px solid #C8D0D0; border-radius:10px; padding:18px; display:flex; flex-direction:column; gap:12px;">
        ...
        <label style="font-size:0.82rem; color:#4A5252; font-weight:600;">Destinatario
            <select name="recipient" required style="width:100%; margin-top:4px; padding:9px; border:1px solid #C8D0D0; border-radius:8px; font-size:0.88rem;">
                <option value="all" @selected(old('recipient')==='all')>📢 Tutta la classe ({{ $students->count() }} studenti)</option>
                <optgroup label="Singolo studente">
                    @foreach($students as $s)<option value="{{ $s->id }}" @selected(old('recipient')===$s->id)>{{ $s->name }}</option>@endforeach
                </optgroup>
            </select>
        </label>
