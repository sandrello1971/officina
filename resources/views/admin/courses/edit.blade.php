    ...
    <div style="display:flex; align-items:center; gap:10px; margin-bottom:20px; flex-wrap:wrap;">
        ...
        <div style="margin-left:auto; display:flex; gap:6px;">
            ...
            <a href="{{ route('admin.rag.index', ['course_id' => $course->id]) }}"
               style="padding:6px 12px; background:white; color:#4A5252; border:1px solid #D1D5DB;
                      border-radius:6px; font-size:0.78rem; font-weight:600; text-decoration:none;">
                📄 Carica documenti (Documenti AI)
            </a>
            <a href="{{ route('admin.course-generation.create', $course) }}"
               style="padding:6px 12px; background:#FDECE2; color:#C26A2E; border:1px solid #E28A53;
                      border-radius:6px; font-size:0.78rem; font-weight:600; text-decoration:none;">
        ...
        <form method="POST" action="/courses/{{ $course->id }}" id="edit-course-form" enctype="multipart/form-data">
            ...
            <div style="display:grid; gap:16px;">
                ...
                <div>
                    ...
                    @php $curMod = old('modality', $course->modality); @endphp
                    <select name="modality"
                            style="width:100%; padding:10px 14px; border:1px solid #C8D0D0; border-radius:8px; font-size:0.875rem; outline:none;">
                        <option value="" @selected($curMod===null || $curMod==='')>Non impostata (contano tutte le ore)</option>
                        <option value="async" @selected($curMod==='async')>Asincrono (FAD) — contano le ore sui moduli</option>
                        <option value="sync" @selected($curMod==='sync')>Sincrono (aula/webinar) — contano le presenze alle sessioni</option>
                    </select>
                ...
                @include('admin.courses._taxonomy-fields', [
                    ...
                    'tags'           => $tags,
                    'selectedCat'    => old('course_category_id', $course->course_category_id),
